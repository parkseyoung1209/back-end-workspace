...
*/
-- 각 부서별 조회
SELECT dept_code, count(*) "사원 수" , format(sum(salary), 0) "총 급여", format(avg(salary), 0) "평균 급여", min(salary) "최저 급여", max(salary) "최고 급여"
FROM employee
GROUP BY dept_code;
...
-- 성별(남자/여자)별 사원 수 조회
-- SELECT if(emp_no like "%-1%", "남자", "여자") as 성별, count(*)
-- SELECT if(substr(emp_no, 8, 1) = 1, "남자", "여자") as 성별, count(*)
...
/*
    ...
    5 SELECT * | 조회하고자 하는 컬럼 명 AS 별칭 | 함수
...
-- 부서별 평균 급여가 300만원 이상이 부서의 평균 급여 조회

SELECT dept_code, format(truncate(avg(salary), -1), 0) as 평균급여
FROM employee
GROUP BY dept_code
HAVING avg(salary) >= 3000000;

-- 직급별 총 급여의 합이 1000만원 이상인 직급만 조회
SELECT job_code, sum(salary) as 총직급
FROM employee
GROUP BY job_code
HAVING 총직급 >=10000000;
-- 부서별 보너스를 받는 사원이 없는 부서만 조회
SELECT dept_code, avg(bonus) AS nobonus
FROM employee
GROUP BY dept_code
HAVING nobonus is null;
-- 또는
...
SELECT dept_code, avg(bonus) AS bonus, count(*) as 사원수
FROM employee
GROUP BY dept_code
