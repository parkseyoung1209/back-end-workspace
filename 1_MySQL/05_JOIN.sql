/*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
      ...
      부서 데이터는 부서 테이블, 사원에 대한 테이블, 직급 테이블 등...
      
      사용예시
      어떤 사원이 어떤 부서에 속해있는지 부서명과 같이 조회하고 싶다면?
...
/*
	1. 내부 조인(INNER JOIN)
    - 연길시키는 컬럼의 값이 일치하는 행들만 조인되어 조회(일치하는 값이 없는 행은 조회 X)
    ...
    2) ANSI(미국국립표준협회, 산업 표준을 제정하는 단체) 표준 구문
    SELECT 컬럼, 컬럼, ...
    FROM 테이블1
    INNER JOIN 테이블2 ON (테이블1.컬럼명 = 테이블2.컬럼명);
    
    -FROM 절에서 기준이 되는 테이블을 기술
    - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭시킬 컬럼에 대한 조건을 기술(ON, USING)
    - 연결에 사용하려는 컬럼명이 같은 경우 ON 구문 대신 USING(컬럼명) 구문 사용
*/
-- 1) 연결할 두 컬럼명이 다른 경우
-- 사번, 사원명, 부서코드, 부서명(dept_title) 조회
...
-- 3) FULL JOIN : 두 테이블이 가진 모든 행을 조회할 수 있음 - MySQL 제공 X
...
-- 사번, 사원명, 부서명, 직급명 조회
SELECT *
FROM employee; -- emp_id, emp_name      foreign-key(외래키):dept_code, job_code
SELECT *
FROM department; -- dept_title      primary-key(주요키):dept_id
SELECT *
FROM job; -- job_name,         primary-key(주요키):job_code

SELECT emp_id, emp_name, dept_title, job_name
FROM employee
JOIN department ON (dept_code = dept_id)
JOIN job USING (job_code); -- 여러 개의 테이블을 join할 때 join을 이중 사용한다
...
JOIN job USING(job_code)
JOIN department ON (dept_code = dept_id)
WHERE substr(emp_no,1, 2) >=70 and 
substr(emp_no,1, 2) <80  and 
substr(emp_name, 1,1) = "전"  and 
substr(emp_no, 8,1) in ('2', '4');
-- WHERE emp_no LIKE '7_____-2%'

-- 3. 보너스를 받은 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역 조회
SELECT emp_name, bonus, format((salary+salary*bonus)*12 ,0) as 연봉, dept_title, local_name
FROM employee
left JOIN department ON (dept_code = dept_id)
left JOIN location ON (location_id = local_code)
-- 부서가 없는 직원들도 나타내고 싶다면 LEFT JOIN (현재는 employee 테이블이 왼쪽에 있음)
WHERE bonus is not null;
...
SELECT dept_title, format(truncate(avg(salary),-1),0) as "부서 평균 급여"
FROM employee
JOIN department on (dept_code = dept_id)
group by dept_title;

-- 6. 각 부서별 총 급여의 합이 1000만원 이상인 부서명, 급여 합 조회
SELECT dept_title, format(sum(salary),0) as "부서 급여 합"
FROM employee
JOIN department on (dept_code = dept_id)
group by dept_title
...
-- 급여 등급이 S1, S2인 경우 고급 => 5999999원 이상 1000만원 이하
-- 급여 등급이 S3, S4인 경우 중급 => 300만원 이상 4999999원 이하
-- 급여 등급이 S5, S6인 경우 초급 => 2999999원 이상 100만원 이하

SELECT emp_id, emp_name, job_name, salary, 
if(sal_level in ('s1', 's2'), "고급", 
if(sal_level in ('s3', 's4'), "중급", "초급")) as 급여등급, 
CASE WHEN sal_level IN ('S1', 'S2') THEN '고급'
	 WHEN sal_level IN ('S3', 'S4') THEN '중급'
     ELSE '초급'
     END AS '급여등급(CASE문 사용)'
FROM employee
JOIN job USING (job_code)
JOIN sal_grade ON (min_sal <= salary AND salary <= max_sal);
...
JOIN department ON (dept_code = dept_id)
JOIN location ON (location_id = local_code);
-- WHERE dept_code is not null; <- JOIN에서 이미 NULL 값을 제외시키기 때문에 굳이 필요가 없다.

-- 10. 해외영업팀에 근무하는 직원들의 직원명, 직급명, 부서코드, 부서명 조회
SELECT emp_name, job_name, dept_title, dept_code, national_name -- national_name은 확인용으로 넣음
FROM employee
JOIN job USING (job_code)
JOIN department ON (dept_code = dept_id)
JOIN location ON (location_id = local_code)
...
-- 12.사번, 사원명 부서명 직급명 지역명 국가명 급여등급 조회 참고로 임플로이 잡 디파트먼트 로케이션 네셔널 샐그레이드
SELECT emp_name, dept_title, job_name, local_name, national_name, sal_level
FROM employee
JOIN department ON (dept_code = dept_id)
JOIN job USING (job_code)
JOIN location ON (location_id = local_code)
