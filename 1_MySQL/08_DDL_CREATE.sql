/*
    ...
    - 객체(Object)/스키마(Schema)를 만들고(CREATE), 변경하고(ALERT), 삭제(DROP)하는 언어
...
/*
    ...
    * 자료형
    1. 문자
		- CHAR(2) / VARCAHR : 고정 및 가변 길이 문자, 반드시 크기 지정
        - TEXT : 매우 긴 문자열 저장하는데 사용
	2. 숫자
		- INT : 정수값 저장하는데 사용
	...
	5. 이진 자료형
		- BLOB : 이진 데이터를 저장하는데 사용, 이미지나 동영상과 같은 이전 파일
...
CREATE TABLE member(
    ...
    INSERT INTO member 
    VALUES(1, 'user01', 'pass01', '공재욱', '남', '010-7310-4769', 'aaa@google.com', '2024-05-28');
    INSERT INTO member 
    ...
    */
    ...
    mem_name VARCHAR(20) NOT NULL, -- 만들고자 하는 컬럼에 붙여주면 된다
    gender CHAR(3),
    phone VARCHAR(13),
    email VARCHAR(50),
    mem_date DATE
    ...
    INSERT INTO mem_notnull VALUES(1, 'user01', 'pass01', '채승훈', '남', null, null, null);
    INSERT INTO mem_notnull VALUES(2, 'user02', null, '손정배', '남', null, null, null);
    -- >> NOT NULL 제약조건에 위배되어 오류 발생!
     INSERT INTO mem_notnull VALUES(2, 'user01', 'pass01', '손정배', '남', null, null, null);
    ...
    /*
		...
		- 추가/수정 시 기존에 있는 데이터값 중 중복값이 있을 경우
    */
    ...
    INSERT INTO mem_unique VALUES(2, 'user01', 'pass02', '주준영', '남', null, null, null);
...
CREATE TABLE member(
    ...
    grade_id INT, -- > 회원등급, 번호 같이 보관할 컬럼
...
/*
	...
	--> 다른 테이블을 참조한다고 표현
    --> 주로 FOREINGN KEY 제약조건에 의해 테이블 간의 관계가 형성됨!
*/
CREATE TABLE member(
	mem_no INT PRIMARY KEY AUTO_INCREMENT,
    mem_id VARCHAR(20) NOT NULL UNIQUE,
...
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
VALUES('user01', 'pass01', '최제한', null);
-- >> 외래키 제약 조건이 부여돼도 기본적으로 null 허용
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
VALUES('user02', 'pass02', '김경주', 1);
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
VALUES('user03', 'pass03', '우현성', 4);
-- >> 부모키를(parent key)를 찾을 수 없다는 오류 발생!
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
VALUES('user03', 'pass03', '우현성', 2);

SELECT * FROM member;
SELECT * FROM mem_grade;
-- >> 부모 테이블(mem_grade)에서 데이터 값을 삭제할 경우 어떤 문제가 발생하는지!
...
/*
    ...
    - 삭제옵션 : 부모 테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할건지
    
    1. ON DELETE RESTRICTED (기본값)
		: 자식데이터로 쓰이는 부모데이터는 삭제가 아예 안되게끔
	2. ON DELETE SET NULL
		: 부모데이터 삭제시 해당 데이터를 쓰고 있는 자식 데이터의 값을 NULL로 처리
    3. ON DELETE CASCADE
		: 부모데이터 삭제시 해당 데이터를 쓰고 있는 자식데이터도 같이 삭제
    
*/
-- on delete set null 방식
drop table member;
CREATE TABLE member(
...
WHERE grade_code = 1;
-- > 삭제가능! 1을 가져다 쓰고 있는 자식 데이터 값이 null로 변경
...
-- mem-grade 테이블에서 grade_code가 2인 데이터 삭제
DELETE 
FROM mem_grade
WHERE grade_code = 2;
