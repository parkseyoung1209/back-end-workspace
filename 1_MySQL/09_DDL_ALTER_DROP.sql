...
ALTER TABLE dept_copy MODIFY lname VARCHAR(20) DEFAULT NULL;
...
-- 다중 수정 가능!
-- dept_title 컬럼의 자료형은 VARCHAR(40), location_id 컬럼의 자료형은 VARCHAR(2), 
-- lname 컬럼의 기본값을 미국으로 변경 (자료형 : VARCHAR(20))

ALTER TABLE dept_copy MODIFY dept_title VARCHAR(40), MODIFY location_id VARCHAR(2), 
MODIFY lname VARCHAR(20) DEFAULT '미국'; -- modify를 기준으로 여러개 사용 가능
...
ALTER TABLE mem_grade DROP COLUMN grade_gode;
...
/*
    ...
    - ADD [CONSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES 테이블명(컬럼영);
...
/*
    ...
    - PRIMARY KEY : ALTER TABLE 테이블명 DROP PRIMARIY KEY;
...
DROP TABLE mem_grade; -- 부모
DROP TABLE member; -- 자식
