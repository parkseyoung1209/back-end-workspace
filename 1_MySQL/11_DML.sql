/*
    ...
    -데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나, 수정하거나(UPDATE), 삭제하는 구문(DELETE)
	...
	2) INSERT INTO 테이블명(컬럼명, 컬럼명, 컬럼명, ...) VALUES (값, 값, ...)
	...
	3> INSERT INTO 테이블명 서브쿼리;
		- VALUES 대신 서브쿼리 조회한 결과값이 통째로 INSERT한다.
        - 서브쿼리 결과값이 INSERT문에 지정된 테이블 컬럼 개수와 같다.
*/
-- 사용할 테이블 생성
CREATE TABLE emp(
...
FROM kh.employee
WHERE 1 = 0; -- WHERE False로 대체 가능

SELECT * FROM emp_dept;
SELECT * FROM emp_manager;

-- emp_dept 테이블에서 employee 부서코드가 D1인 직원의 사번, 이름, 부서코드, 입사일 추가하고
...
/*
    ...
    - 사실 서브쿼리 사용 가능! 그러나 잘 쓰이지도 않을 뿐더러 버전마다 상황이 다르다.
*/
...
SET emp_id = null
WHERE emp_id = 201;
-- >> emp_id primary key 제약조건 위배(not null 위배)
...
ROLLBACK; -- DELETE는 ROLLBACK 가능! , TRUNCATE는 ROLLBACK 불가능!
