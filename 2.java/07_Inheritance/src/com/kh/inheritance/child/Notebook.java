 ...
 * 	->Object 클래스의 매서드들을 오버라이딩 하여 매서드 재정의 가능
...
public class Notebook extends Product {
	...
	public Notebook(String brand, String pCode, String name, int price) {
	...
	public Notebook(String brand, String pCode, String name, int price, String cpu) {
		...
		//super.pCode = pCode; // 부모가 default - 같은 페키지에 있어야만 직접 접근
		super.name = name; // 부모가 protected - 상속관계일 때 자식 객체에서 직접 접근
		//super.price = price; // 부모가 private -직접 접근 x
		
		// 3. 부모의 setter 매서드를 호출해서 초기화
		// 	  - 부모 클래스에서 setter 매서드를 제공하면
	...
	/*
	 * 오버라이딩(overriding)
	 * - 자식 클래스가 상속받은 부모 클래스의 매서드를 재정의하는것
	 */
