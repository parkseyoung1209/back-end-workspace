 ...
 *  
 *  * ArrayList
 *  - 저장 용량(capacity)를 초과한 객체들이 들어오면 자동적으로 저장 용량이 늘어난다.
...
//		list.add(3);
	...
	public void method2() {
		...
		// 4. size : 리스트 안에 몇 개의 데이터가 있는지를 볼 수 있다.
		...
		List<Person> sub = list.subList(0, 3); // 0번째부터 2번째까지 가져온다는 뜻
		...
		for(int i = 0; i<list.size(); i++) {
			result =  result + (list.get(i).getAge());
		}
	...
	public void method3() {
		...
		// reverse 자체의 기능은 배열을 역순으로만 정렬하는 것이기 때문에 sort가 필수이다.
