 ...
 * - anyMatch : 최소한 한개의 요소가 매개 값으로 주어진 조건을 만족하는지 조사
...
public class A_Match {
	public void method1() {
		...
		Arrays.stream(values).noneMatch(value -> value %5 == 0);
	...
	public void method2() {
		...
		boolean one = students.stream()
//			.filter(student -> student.getAge() >= 20)
//			.allMatch(student -> student.getGender().equals("남자"));
				.allMatch(student -> student.getAge() >= 20 && student.getGender().equals("남자"));
		System.out.println(one);
		
		// 남학생들 중에 평균이 70점 이상인 학생들이 존재하는지 확인
		boolean two = students.stream()
