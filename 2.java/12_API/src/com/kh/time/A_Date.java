...
public class A_Date {
	 ...
	 * - Date의 매서드는 대부분 (deprecated(비권장) 되어있지만 여전히 사용
	 */
	public void method1() {
		// 기본 생성자로 객체를 생성하면 현재 시스템의 시간에 대한 정보를 가지고 객체 생성
		Date today = new Date();
		System.out.println(today); // Thu Jul 11 11:46:50 KST 2024
		
		Date when = new Date(0);
		System.out.println(when); // Thu Jan 01 09:00:00 KST 1970
		
		System.out.println("getTime : " + today.getTime()); //1720666196683 == Thu Jul 11 11:49:56 KST 2024
	...
	public void method2() {
		/*
		 * 추상 클래스이기 때문에 직접 객체 생성할 수가 없다.
		 * getInstance() 매서드로 Calendar 클래스의 객체를 반환
